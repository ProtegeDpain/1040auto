...
async function main() {
    ...
    await prisma.users.upsert({
        ...
        create: {
            ...
            username: 'admin@example.com', // username set to email
    ...
    // Seed TaskFiles
    let taskFile = await prisma.taskFiles.findFirst({ where: { original_filename: 'document.pdf', task_id: task.id } });
    if (!taskFile) {
        await prisma.taskFiles.create({
            data: {
                task_id: task.id,
                file_type: 'PDF',
    ...
    // Seed TaskStepErrors
    let taskStepError = await prisma.taskStepErrors.findFirst({ where: { error_code: 'ERR001', task_id: task.id } });
    if (!taskStepError) {
    ...
    for (const name of filingStatuses) {
